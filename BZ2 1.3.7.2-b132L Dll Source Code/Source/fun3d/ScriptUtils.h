...
// See GiveWeapon(Handle me, Name weapon, int Slot); below for a version that overrides normal checks.
...
// use. The returned string has .odf appended.
...
// NOTE: This function is vestigial, as it was recently added and thus not used by any existing code, 
// but the writer forgot the * on char* (or Name), so it can't actually accept a full weapon odf e.g. "gminigun_c".
// It could only accept 'g', the chances of a user creating a weapon odf 1 character long is minute, 
// best to remove/ignore and use the version below instead.
...
// which must be [0 .. 5]. No normal carrier restrictions are really
// checked, it just stomps over the weapon. 
DLLEXPORT void DLLAPI GiveWeapon(Handle h, int Slot, char* Weapon); 
